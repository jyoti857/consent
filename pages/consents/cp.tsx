...
const CommunicationPreferences: React.FC<CommunicationPreferencesProps> = ({
  ...
  const adcConsent =
    consents && consents.find((conse: any) => conse.preference.type === "UNBR");
  const commConsent =
    consents && consents.find((conse: any) => conse.preference.type === "BRAN");
  ...
  const [adc, setAdc] = React.useState(
    adcConsent
      ? adcConsent.preference.subscribed
        ? "adc_sub"
        : "adc_unsub"
      : ""
  );
  const [comm, setComm] = React.useState(
    commConsent
      ? commConsent.preference.subscribed
        ? "comm_sub"
        : "comm_unsub"
      : ""
  );
  ...
  const handleFormSubmit = async (e: any) => {
    ...
    const data = {
      ...
      lastName: lastName_,
      state: state_ || initialValues.state,
      type: type_ || initialValues.type,
      adc,
      comm,
    };
    ...
    <div>
      ...
      >
        <CardContent>
          <div style={{ display: "flex" }}>
            <div>
              ...
              >
                ADCT respects your privacy. Please see our Privacy Notice for
                more information. Complete the information below to manage your
                email communication preferences from ADC Therapeutics.
              </Typography>
            </div>
            <div style={{ width: "35%" }}>
              <CardMedia
        ...
        <div>
          ...
          >
            <FormLabel style={{ color: "#686868" }}>
              ADC Therapeutics Corporate Email and News*
            </FormLabel>
            <RadioGroup value={adc} onChange={handleAdcPreferences}>
              <FormControlLabel
                value="adc_sub"
                control={<Radio style={{ color: "#00A7E1" }} />}
                label="Subscribe"
              />
              <FormControlLabel
                value="adc_unsub"
            ...
            <FormLabel style={{ color: "#686868" }}>
              Communications from Sales and Marketing*
            </FormLabel>
        ...
        <div style={{ margin: 18 }}>
          ...
          >
            ADC Therapeutics America, Inc. (“ADCT”) understands protecting your
            personal information is very important. We do not share any
            personally identifiable information you give use with third parties
            for their own marketing use. I understand by selecting "Subscribe”
            and clicking "Submit" below, that the information provided by me may
            be used for marketing purposes by ADCT in order to keep me informed
            about its products, services, promotional education or other
            opportunities that may be of interest to me via email. I can opt out
            of marketing communications at any time by clicking the link in any
            email I receive, or by sending a letter with my opt out request to
            430 Mountain Avenue, Suite #404, New Providence, NJ 07974.
          </Typography>
        </div>
        <CardActions>
          <Button
...
export const getServerSideProps = async (ctx: any) => {
  ...
  const d = await (await dbConnect())
    .collection("pending_consent_user")
    .findOne({ _id: new ObjectId(ctx.query.token) });
  console.log("d --->", d);
  const stringi = JSON.stringify(d);
  const parse = JSON.parse(stringi);
  if (parse && parse._id) {
    const existing_user = await (await dbConnect())
      .collection("consent_user")
      .findOne({ _id: new ObjectId(parse._id) });
