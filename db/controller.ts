...
export const activateUser = async (req: any, res: any) => {
  ...
  console.log(
    "query hash from activateuser api --->",
    hash,
    firstName,
    lastName,
    state,
  ...
  try {
    ...
    if (data) {
      ...
      const isConsentCollectionExist = await req.db
        .collection("consents")
        .find({ hcpId: hash })
        ?.limit(1);
      console.log(
        "is consent collection exist ----> ",
        isConsentCollectionExist
      );
      if (!isConsentCollectionExist.length) {
        console.log("in if ");
        const adcPrefCollection_ = await req.db
          .collection("ADC_preference")
          .insertOne(adcPreference);
        const adcPrefCollection = adcPrefCollection_.ops[0];
        console.log("adcPrefCollection ---> ", adcPrefCollection);
        const commPrefCollection_ = await req.db
          .collection("COMM_preference")
          .insertOne(commPreference);
        const commPrefCollection = commPrefCollection_.ops[0];
        console.log("commPrefCollection ---> ", commPrefCollection);
        consentCollection = await req.db.collection("consents").insertMany([
          {
            ...adcObj,
            preferenceId: adcPrefCollection._id,
            preference: {
              ...adcPrefCollection,
              subscribed: adc === "adc_sub" ? true : false,
            },
          },
          {
            ...commObj,
            preferenceId: commPrefCollection._id,
            preference: {
              ...commPrefCollection,
              subscribed: comm === "comm_sub" ? true : false,
            },
          },
        ]);
        ...
        console.log("in else");
        const getConsentsByUser = await req.db
          .collection("consents")
          .find({ hcpId: hash })
          .toArray();
      ...
      console.log(
        "*** && consents from backend ---> ",
        consentCollection.ops[0]
      );
      await req.db.collection("consent_user").insertOne(
        // {_id: new ObjectId(hash)},
        {
          ...data,
          name: `${firstName.toString().trim()} ${lastName.trim()}`,
          state,
          hcpType: type ? type : "Other",
      ...
      const addConsDoc = await req.db
        .collection("consents")
        .findOne({ preferenceId: adcId });
      const commConsDoc = await req.db
        .collection("consents")
        .findOne({ preferenceId: commId });
      console.log("addConsDoc, commConsDoc --> ", addConsDoc, commConsDoc);
      // in progress
      await req.db.collection("consents").updateOne(
        { preferenceId: adcId },
        {
          $set: { "preference.subscribed": adc === "adc_sub" ? true : false },
        }
      );
      await req.db.collection("consents").updateOne(
        { preferenceId: commId },
        {
          $set: {
            "preference.subscribed": comm === "comm_sub" ? true : false,
          },
        }
      );
      const data_ = {
      ...
      await req.db
        .collection("consent_user")
        .updateOne({ _id: data._id }, { $set: { ...data_ } });
...
export const findMovies =
  (limit: number) => async (req: Document, res: any) => {
    let doc = await req.db.collection("movies").find().limit(limit).toArray();
    res.json(doc);
  };

export const findOneMovie =
  (title: string) => async (req: Document, res: any) => {
